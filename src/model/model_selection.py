...
from sklearn.model_selection import KFold, cross_validate, StratifiedKFold, GridSearchCV, train_test_split
from sklearn.pipeline import Pipeline
from sklearn.linear_model import LogisticRegression
from sklearn.preprocessing import StandardScaler
from sklearn.ensemble import AdaBoostClassifier, ExtraTreesClassifier, GradientBoostingClassifier, GradientBoostingRegressor, RandomForestClassifier, RandomForestRegressor
...
class Splitter():
    ...
    def __init__(self, df:pd.DataFrame, features_col:list, target_col:str, 
                random_state:int, test_size:float=0.25):
        """Data Splitter class
...
class Classifier(Splitter):
    def __init__(self, model:object, df:list, features_col:list, target_col:str, model_type:str,
    ...
    def _metrics(self,)->dict:
        if self.model_type == 'classification':
            ...
            metrics = {
                ...
                'Mean Absolute Error': mean_absolute_error(self.test_target, self.predictions),
                'Mean Squared Error': mean_squared_error(self.test_target, self.predictions),
                }
        return metrics
...
class ModelSelection(Splitter):
    def __init__(self, df:pd.DataFrame, features_col:list, target_col:str, model_type:str,
    ...
    def get_best_model(self, outdir:str=None, selection_metric:str=None)->str:
        ...
        for model, config in self._model_configurations().items():
            if config['enable']:
                gcv = GridSearchCV(estimator=config[self.model_type],
    ...
    def save_results(self, outdir:str)->None:
        self.results.to_csv(os.path.join(outdir, 'model_selection.tsv.gz'), 
    ...
    def _inner_cv(self, shuffle:bool=True)->StratifiedKFold:
        if self.model_type == 'classification':
            ...
            cv = KFold(n_splits=self.n_outer_splits, shuffle=shuffle, random_state=self.random_state) 
        return cv
    ...
    def _model_configurations(self):
        grid = {
            'AdaBoost': {
                'enable': False,
                'shortname': 'AB',
                'classification': AdaBoostClassifier(n_estimators=400, random_state=self.random_state),
                'regression': "None",
            ...
            'Decision Tree': {
                'enable': True,
                'shortname': 'DT',
                'classification': DecisionTreeClassifier(random_state=self.random_state),
            ...
            'Extremely randomized trees': {
                'enable': False,
                'shortname': 'ET',
                'classification': ExtraTreesClassifier(n_estimators=400, random_state=self.random_state),
                'regression': "None",
            ...
            'K-Nearest Neighbors': {
                'enable': False,
                'shortname': 'KNN',
                'classification':  Pipeline([('std', StandardScaler()),
            ...
            'Random Forest': {
                'enable': True,
                'shortname': 'RF',
                'classification': RandomForestClassifier(n_estimators=1000, random_state=self.random_state, n_jobs=10),
                'regression': RandomForestRegressor(n_estimators=1000, random_state=self.random_state, n_jobs=-1),
                'grid1': [{'min_samples_leaf': list(range(1, 10)),
                        }]
            ...
            'Gradient Boosting Machine': {
                'shortname': 'GBM',
                'enable': True,
                'classification':  GradientBoostingClassifier(n_estimators=1000, random_state=self.random_state),
                'regression': GradientBoostingRegressor(n_estimators=1000, random_state=self.random_state),
                'grid1': [{'min_samples_leaf': list(range(1, 10)),
            ...
            'XGBoost': {
                'shortname': 'XGB',
                'enable': True,
                'classification': XGBClassifier(scale_pos_weight=4, seed=self.random_state),
                'regression': XGBRegressor(n_estimators=1000, seed=self.random_state),
