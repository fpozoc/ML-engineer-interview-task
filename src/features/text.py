...
Usage:
    python -m src.features.text -d data/raw/immo_data.csv -o data/processed/immo_data.translated.description.tsv.gz
"""
...
def main():
    ...
    parser.add_argument("-o", "--output", help="Training set.")
...
def nlp_feature_engineering(df: pd.DataFrame, features: list) -> pd.DataFrame:
    ...
    for feature in features:
        ...
        df[f"{feature}_finbert"] = [
            huggingface_sentiment_analysis(pipeline=model, my_sentence=str(sentence))
            for sentence in df[feature]
        ]

        # VADER
        df[f"{feature}_vader"] = [
            vader_sentiment_analysis(my_sentence=str(sentence))
            for sentence in df[feature]
        ]

        # TextBlob
        df[f"{feature}_textblob"] = [
            textblob_sentiment_analysis(my_sentence=str(sentence))
            for sentence in df[feature]
        ]
        df[f"{feature}_textblob_polarity"], df[f"{feature}_textblob_subjectivity"] = df[
            f"{feature}_textblob"
        ].str
...
def batch_translator(text_list: list) -> str:
    ...
    text_list = list(
        map(
            ...
            [
                re.sub("[^a-zA-Z0-9 \.]", "", str(my_sentence))
                for my_sentence in text_list
            ],
        )
    )
